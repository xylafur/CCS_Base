...
m_ipAddressCombo.addRun(new MyRunnable());
		...
		m_ipAddressButton.addRun(new IRun({run: function()
		{
			...
			if (parity != null)
			{
				parity = new java.lang.String(parity);
...
var MyRunnable = Java.extend(Runnable, { 
	run: function() { 
		var m_speed = null;
			...
			{
				regexComPort = "/dev/tty\\.usbmodem(\\w+)?:?(\\d+)?[/\\-\\.]?([78])?([NEOMS])?([12])?";
			}
			...
			{
				m_portName.setText("/dev/tty.usbmodem"+m.group(1));
			}
			else
			{
				m_portName.setText("COM"+m.group(1));
...
m_saveDataFile.addRun(new MyRunnable());
		...
		*/
		...
		else
		{
			...
			m_collectDuration.setText(new java.lang.Integer(m_duration).toString());
		...
		else
		{
			...
			m_maxDataEdit.setText(new java.lang.Integer(m_maxData).toString());
		} 
